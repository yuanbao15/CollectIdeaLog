...
# 修复1：增强获取日志路径，解决通配符报错问题
# 查找最新版本的IntelliJIdea日志文件
$logPattern = "$env:USERPROFILE\AppData\Local\JetBrains\IntelliJIdea*\log\idea.log"
# 查找符合模式的日志文件
$logPaths = Get-ChildItem -Path $logPattern -ErrorAction SilentlyContinue
if (-not $logPaths) {
    Write-Host "[ERROR] can't find IntelliJ log, please confirm have opened the IDE!" -ForegroundColor Red
    exit 1
}
$logPath = $logPaths[0].FullName # 假设只处理找到的第一个日志文件
